...
/*  Bit Manipulation: Time = O(n) space = O(1)
    ...
    x = a XOR b: 3 ^ 5 = 110(6)；有两个位不相同
    制造分离判断因子：找出 a 和 b 二进制表示法中不相同的其中一位
    x = x & (~(x-1)) -> 010；这会找到 x 从右往左数第一个为 1 （即 a 和 b 在这个位上不相同）的位
    将所有数分为 &x == 0 和 &x != 0 两组：[1,1,5]/[2,2,3]；每组内部进行 XOR 运算
 */
public class SingleNumber3 {
    public int[] singleNumber(int[] nums) {
        ...
        int lastBit = diff & ~(diff - 1);   // 负数相当于取反+1 -> diff & -diff
        int[] res = new int[2];
        for (int num : nums) {
            if ((num & lastBit) == 0) {
                res[0] ^= num;
