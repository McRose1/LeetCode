...
想到 backtrack函数返回类型为 void，然后在里面构建一个 List<Integer> list，每次res.add(list的deep copy)
                Root
          /       |         \
        1         2           3
       / \       / \         / \
...
如何限制每个元素只用一次？
一开始想到用boolean数组来记录元素是否被 used，结果报出了 stackOverflowError
