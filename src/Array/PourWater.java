...
/*  755. Pour Water
    ...
    Water first drops at index K and rests on top of the highest terrain of water at that index.
    Then, it flows according to the following rules:
    o If the droplet would eventually fall be moving left, then move left.
    o Otherwise, if the droplet would eventually fall by moving right, then move right.
    o Otherwise, rise at it's current position.
...
/*  Simulation: Time = O(n^2) Space = O(1)
    Find the lowest slot in a non-increasing sequence.
 */
public class PourWater {
    ...
    private void helper(int[] heights, int K) {
        int best = K;
        // -1 表示向左走，1 表示向右走
        for (int d = -1; d <= 1; d += 2) {
            int i = K + d;
